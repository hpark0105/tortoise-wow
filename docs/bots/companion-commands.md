...
| `.botdismiss <botname>` | Removes the bot from your party. |
...
| `.botstop <botname>` | Cancels the follow order. With no other active order, the bot resumes legacy idle behavior: local wandering and auto-hunting nearby mobs. It does NOT seek you back out; re-issue `.botfollow` to bring it back. |
...
- With **no** active order at all, the bot runs the legacy loop: wander
...
- Death and recovery: if the bot dies while it has an active order
  ...
  no second mechanism). If it dies with **no** active order (legacy
  auto-hunt), it stays dead-idle at the corpse until you give it an order or
  recall it.
...
## Typical session flow
    ...
    .botstop Companion            # cancel follow; it roams and hunts on its own
